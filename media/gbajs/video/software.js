...
GameBoyAdvanceSoftwareRenderer.prototype.freeze = function() {
	var paletteBuffer = new Uint16Array(512);
	for (var i = 0; i < 256; ++i) {
		paletteBuffer[i] = this.palette.colors[0][i] || 0;
		paletteBuffer[i + 256] = this.palette.colors[1][i] || 0;
	}
	return {
		'vram': this.vram.buffer.buffer,
		'oam': this.oam.buffer.buffer,
