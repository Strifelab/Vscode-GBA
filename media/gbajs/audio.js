...
GameBoyAdvanceAudio.prototype.updateTimers = function() {
	...
	while (this.enabled) {
		var previousNextEvent = this.nextEvent;
		var previousNextSample = this.nextSample;
		...
		if (cycles >= this.nextSample) {
			...
			if (!(this.nextSample > cycles || this.sampleInterval > 0)) {
				this.nextSample = cycles + 1;
			}
		}
		...
		if (this.nextEvent === previousNextEvent && this.nextSample === previousNextSample) {
			this.nextEvent = Math.max(this.nextEvent, cycles);
			this.nextSample = cycles + this.sampleInterval;
			break;
		}
